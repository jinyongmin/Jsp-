...
<body>
    ...
    <form action="./info_ok.jsp" name="regform" id="regform" method="post" onsubmit="return sendit()">
        ...
        <p>휴대폰번호: <input type="text" name="hp" id="hp" value="<%=member.getHp()%>"></p>
        <p>이메일: <input type="text" name="email" id="email" value="<%=member.getEmail()%>"></p>
        <p>성별:
        	<label>남자<input type="radio" name="gender" value="남자"
        	<% if(member.getGender().equals("남자")) out.print("checked"); %>></label>
        	<label>여자<input type="radio" name="gender" value="여자"
        	<% if(member.getGender().equals("여자")) out.print("checked"); %>></label></p>
        <p>취미:
        <label>등산<input type="checkbox" name="hobby" value="등산"
        <% if(compareHobby(member.getHobby(), "등산")) out.print("checked"); %>></label>
        <label>게임<input type="checkbox" name="hobby" value="게임"
        <% if(compareHobby(member.getHobby(), "게임")) out.print("checked"); %>></label>
        <label>영화감상<input type="checkbox" name="hobby" value="영화감상"
        <% if(compareHobby(member.getHobby(), "영화감상")) out.print("checked"); %>></label>
        <label>드라이브<input type="checkbox" name="hobby" value="드라이브"
        <% if(compareHobby(member.getHobby(), "드라이브")) out.print("checked"); %>></label>
        <label>운동<input type="checkbox" name="hobby" value="운동"
        <% if(compareHobby(member.getHobby(), "운동")) out.print("checked"); %>></label>
        </p>
        <p>주민등록번호: <input type="text" name="ssn1" value="<%=member.getSsn1()%>"> - <input type="text" name="ssn2" value="<%=member.getSsn2()%>"></p>
        <p>우편번호: <input type="text" name="zipcode" maxlength="5" id="sample6_postcode" value="<%=member.getZipcode()%>"> <button type="button" onclick="sample6_execDaumPostcode()">검색</button></p>
